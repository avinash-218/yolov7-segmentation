...
def run(
        data=ROOT / 'data/custom_wall.yaml',  # 'dataset.yaml path'
        weights=ROOT / 'walls.pt',  # weights path
...
def main(opt):
    ...
    for opt.weights in (opt.weights if isinstance(opt.weights, list) else [opt.weights]):
        model_path = run(**vars(opt))

    input_height = opt.imgsz[0]
    input_width = opt.imgsz[1]
    model_path = model_path[0]
    # print(model_path[0])
    # print(model_path[0].split(".onnx"))
    new_name = model_path.split(".onnx")
    os.rename(model_path, f"{new_name[0]}.onnx")
...
# python pytorch2onnx.py --weights walls.pt --include onnx